...
(function(mod) {
  ...
  eronelit_editorv3.defineMode("ebnf", function (config) {
    var commentType = {slash: 0, parenthesis: 1};
    var stateType = {comment: 0, _string: 1, characterClass: 2};
    var bracesMode = null;

    if (config.bracesMode)
      bracesMode = eronelit_editorv3.getMode(config, config.bracesMode);
      ...
      token: function (stream, state) {
        ...
        if (bracesMode !== null && (state.braced || peek === "{")) {
          if (state.localState === null)
            state.localState = eronelit_editorv3.startState(bracesMode);
  ...
  eronelit_editorv3.defineMIME("text/x-ebnf", "ebnf");
});
