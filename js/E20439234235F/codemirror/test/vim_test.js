...
eronelit_editorv3.Vim.suppressErrorLogging = true;
...
function testVim(name, run, opts, expectedFail) {
  ...
  return test('vim_' + name, function() {
    ...
    var cm = eronelit_editorv3(place, vimOpts);
    var vim = eronelit_editorv3.Vim.maybeInitVimState_(cm);
    ...
    function doKeysFn(cm) {
      return function(args) {
        ...
        for (var i = 0; i < arguments.length; i++) {
          var result = eronelit_editorv3.Vim.handleKey(cm, arguments[i]);
      ...
      return function(args) {
        ...
        function executeHandler(handler) {
          if (typeof handler == 'string') {
            eronelit_editorv3.commands[handler](cm);
          } else {
            handler(cm);
          }
          return true;
        ...
        for (var i = 0; i < arguments.length; i++) {
          ...
          var handled = eronelit_editorv3.lookupKey(key, cm.getOption('keyMap'), executeHandler, cm);
          // Record for insert mode.
          if (handled == "handled" && cm.state.vim.insertMode && arguments[i] != 'Esc') {
            var lastChange = eronelit_editorv3.Vim.getVimGlobalState_().macroModeState.lastInsertModeChanges;
            if (lastChange && (key.indexOf('Delete') != -1 || key.indexOf('Backspace') != -1)) {
              lastChange.changes.push(new eronelit_editorv3.Vim.InsertModeKey(key));
            }
          }
        }
      }
    ...
    var helpers = {
      ...
      // Keys are in eronelit_editorv3 format, NOT vim format.
      ...
      getRegisterController: function() {
        return eronelit_editorv3.Vim.getRegisterController();
      }
    }
    eronelit_editorv3.Vim.resetVimGlobalState_();
...
testVim('/_2_pcre', function(cm, vim, helpers) {
  eronelit_editorv3.Vim.setOption('pcre', true);
...
testVim('/_2_nopcre', function(cm, vim, helpers) {
  eronelit_editorv3.Vim.setOption('pcre', false);
  cm.openDialog = helpers.fakeOpenDialog('\\(word\\)\\{2}');
  helpers.doKeys('/');
  helpers.assertCursorAt(1, 9);
  helpers.doKeys('n');
...
testVim('ex_write', function(cm, vim, helpers) {
  var tmp = eronelit_editorv3.commands.save;
  var written;
  var actualCm;
  eronelit_editorv3.commands.save = function(cm) {
    written = true;
    actualCm = cm;
  };
  ...
  eronelit_editorv3.commands.save = tmp;
});
...
function testSubstituteConfirm(name, command, initialValue, expectedValue, keys, finalPos) {
  testVim(name, function(cm, vim, helpers) {
    var savedOpenDialog = cm.openDialog;
    var savedKeyName = eronelit_editorv3.keyName;
    var onKeyDown;
    ...
    eronelit_editorv3.keyName = function (e) { return e.key; }
    ...
    try {
      ...
      eronelit_editorv3.keyName = savedKeyName;
      cm.openDialog = savedOpenDialog;
    }
  }, { value: initialValue });
}
...
testVim('set_boolean', function(cm, vim, helpers) {
  eronelit_editorv3.Vim.defineOption('testoption', true, 'boolean');
  // Test default value is set.
  is(eronelit_editorv3.Vim.getOption('testoption'));
  try {
    // Test fail to set to non-boolean
    eronelit_editorv3.Vim.setOption('testoption', '5');
    fail();
  } catch (expected) {}
  // Test setOption
  eronelit_editorv3.Vim.setOption('testoption', false);
  is(!eronelit_editorv3.Vim.getOption('testoption'));
});
...
testVim('set_string', function(cm, vim, helpers) {
  eronelit_editorv3.Vim.defineOption('testoption', 'a', 'string');
  // Test default value is set.
  eq('a', eronelit_editorv3.Vim.getOption('testoption'));
  try {
    // Test fail to set non-string.
    eronelit_editorv3.Vim.setOption('testoption', true);
    fail();
  } catch (expected) {}
  try {
    // Test fail to set 'notestoption'
    eronelit_editorv3.Vim.setOption('notestoption', 'b');
    fail();
  } catch (expected) {}
  // Test setOption
  eronelit_editorv3.Vim.setOption('testoption', 'c');
  eq('c', eronelit_editorv3.Vim.getOption('testoption'));
});
testVim('ex_set_string', function(cm, vim, helpers) {
  eronelit_editorv3.Vim.defineOption('testopt', 'a', 'string');
  // Test default value is set.
  eq('a', eronelit_editorv3.Vim.getOption('testopt'));
  try {
  ...
  eq('c', eronelit_editorv3.Vim.getOption('testopt'));
  helpers.doEx('set testopt=c')
  eq('c', eronelit_editorv3.Vim.getOption('testopt', cm)); //local || global
  eq('c', eronelit_editorv3.Vim.getOption('testopt', cm, {scope: 'local'})); // local
  eq('c', eronelit_editorv3.Vim.getOption('testopt', cm, {scope: 'global'})); // global
  eq('c', eronelit_editorv3.Vim.getOption('testopt')); // global
  // Test setOption global
  helpers.doEx('setg testopt=d')
  eq('c', eronelit_editorv3.Vim.getOption('testopt', cm));
  eq('c', eronelit_editorv3.Vim.getOption('testopt', cm, {scope: 'local'}));
  eq('d', eronelit_editorv3.Vim.getOption('testopt', cm, {scope: 'global'}));
  eq('d', eronelit_editorv3.Vim.getOption('testopt'));
  // Test setOption local
  helpers.doEx('setl testopt=e')
  eq('e', eronelit_editorv3.Vim.getOption('testopt', cm));
  eq('e', eronelit_editorv3.Vim.getOption('testopt', cm, {scope: 'local'}));
  eq('d', eronelit_editorv3.Vim.getOption('testopt', cm, {scope: 'global'}));
  eq('d', eronelit_editorv3.Vim.getOption('testopt'));
});
testVim('ex_set_callback', function(cm, vim, helpers) {
  ...
  eronelit_editorv3.Vim.defineOption('testopt', 'a', 'string', cb);
  // Test default value is set.
  eq('a', eronelit_editorv3.Vim.getOption('testopt'));
  try {
    // Test fail to set 'notestopt'
...
testVim('ex_set_filetype', function(cm, vim, helpers) {
  eronelit_editorv3.defineMode('test_mode', function() {
    return {token: function(stream) {
      stream.match(/^\s+|^\S+/);
    }};
  });
  eronelit_editorv3.defineMode('test_mode_2', function() {
    return {token: function(stream) {
      stream.match(/^\s+|^\S+/);
    }};
  });
...
testVim('ex_map_key2key_visual_api', function(cm, vim, helpers) {
  eronelit_editorv3.Vim.map('b', ':w', 'visual');
  var tmp = eronelit_editorv3.commands.save;
  var written = false;
  var actualCm;
  eronelit_editorv3.commands.save = function(cm) {
...
testVim('ex_imap', function(cm, vim, helpers) {
  eronelit_editorv3.Vim.map('jk', '<Esc>', 'insert');
  helpers.doKeys('i');
  is(vim.insertMode);
  helpers.doKeys('j', 'k');
  is(!vim.insertMode);
  cm.setCursor(0, 1);
  eronelit_editorv3.Vim.map('jj', '<Esc>', 'insert');
  helpers.doKeys('<C-v>', '2', 'j', 'l', 'c');
  var replacement = fillArray('fo', 3);
  cm.replaceSelections(replacement);
  eq('1fo4\n5fo8\nafodefg', cm.getValue());
...
testVim('ex_unmap_api', function(cm, vim, helpers) {
  eronelit_editorv3.Vim.map('<Alt-X>', 'gg', 'normal');
  is(eronelit_editorv3.Vim.handleKey(cm, "<Alt-X>", "normal"), "Alt-X key is mapped");
  eronelit_editorv3.Vim.unmap("<Alt-X>", "normal");
  is(!eronelit_editorv3.Vim.handleKey(cm, "<Alt-X>", "normal"), "Alt-X key is unmapped");
});
...
testVim('ex_api_test', function(cm, vim, helpers) {
  var res=false;
  var val='from';
  eronelit_editorv3.Vim.defineEx('extest','ext',function(cm,params){
    if(params.args)val=params.args[0];
    else res=true;
  });
  helpers.doEx(':ext to');
  eq(val,'to','Defining ex-command failed');
  eronelit_editorv3.Vim.map('<C-CR><Space>',':ext');
