...
eronelit_editorv3.defaults.rtlMoveVisually = true;
...
test("core_fromTextArea", function() {
  ...
  te.value = "CONTENT";
  var cm = eronelit_editorv3.fromTextArea(te);
  is(!te.offsetHeight);
...
test("core_defaults", function() {
  var defsCopy = {}, defs = eronelit_editorv3.defaults;
  for (var opt in defs) defsCopy[opt] = defs[opt];
  defs.indentUnit = 5;
  defs.value = "uu";
  defs.indentWithTabs = true;
  defs.tabindex = 55;
  var place = document.getElementById("testground"), cm = eronelit_editorv3(place);
  try {
...
testCM("markTextMultiLine", function(cm) {
  ...
  forEach([{a: [0, 0], b: [0, 5], c: "", f: [0, 0], t: [2, 5]},
    ...
    var r = cm.markText(Pos(0, 5), Pos(2, 5),
                        {className: "eronelit_editorv3-matchingbracket"});
...
testCM("selectionPos", function(cm) {
  ...
  cm.scrollTo(0, 0);
  var selElt = byClassName(cm.getWrapperElement(), "eronelit_editorv3-selected");
  var outer = cm.getWrapperElement().getBoundingClientRect();
...
testCM("doubleScrollbar", function(cm) {
  ...
  is(wrap.offsetWidth - byClassName(wrap, "eronelit_editorv3-lines")[0].offsetWidth <= scrollbarWidth * 1.5);
});
...
testCM("collapsedLines", function(cm) {
  ...
  eronelit_editorv3.on(range, "clear", function() {cleared++;});
  cm.setCursor(Pos(3, 0));
  eronelit_editorv3.commands.goLineDown(cm);
...
testCM("collapsedRangeBetweenLinesSelected", function(cm) {
  ...
  var selElts = byClassName(cm.getWrapperElement(), "eronelit_editorv3-selected");
...
testCM("hiddenLinesSelectAll", function(cm) {  // Issue #484
  ...
  foldLines(cm, 0, 10);
  foldLines(cm, 11, 20);
  eronelit_editorv3.commands.selectAll(cm);
...
testCM("structuredFold", function(cm) {
  ...
  eronelit_editorv3.commands.goCharLeft(cm);
  eqCharPos(cm.getCursor(), Pos(1, 2));
  eronelit_editorv3.commands.delCharAfter(cm);
  eq(cm.getValue(), "xxxx\nxxxx\nxxxx");
  addDoc(cm, 4, 8);
  ...
  range.clear();
  cm.setCursor(1, 2);
  eronelit_editorv3.commands.goCharRight(cm);
...
testCM("changedInlineWidget", function(cm) {
  ...
  m.changed();
  var hScroll = byClassName(cm.getWrapperElement(), "eronelit_editorv3-hscrollbar")[0];
...
testCM("measureEndOfLine", function(cm) {
  if (phantom) return;
  cm.setSize(null, "auto");
  var inner = byClassName(cm.getWrapperElement(), "eronelit_editorv3-lines")[0].firstChild;
...
testCM("scrollVerticallyAndHorizontally", function(cm) {
  ...
  addDoc(cm, 40, 40);
  cm.setCursor(39);
  var wrap = cm.getWrapperElement(), bar = byClassName(wrap, "eronelit_editorv3-vscrollbar")[0];
  is(bar.offsetHeight < wrap.offsetHeight, "vertical scrollbar limited by horizontal one");
  var cursorBox = byClassName(wrap, "eronelit_editorv3-cursor")[0].getBoundingClientRect();
...
testCM("moveVstuck", function(cm) {
  var lines = byClassName(cm.getWrapperElement(), "eronelit_editorv3-lines")[0].firstChild, h0 = lines.offsetHeight;
...
testCM("extraKeys", function(cm) {
  ...
  eronelit_editorv3.commands.testCommand = function() {outcome = "tc";};
  eronelit_editorv3.commands.goTestCommand = function() {outcome = "gtc";};
...
testCM("rtlMovement", function(cm) {
  ...
  forEach(["خحج", "خحabcخحج", "abخحخحجcd", "abخde", "abخح2342خ1حج", "خ1ح2خح3حxج",
    ...
    var cursors = byClassName(cm.getWrapperElement(), "eronelit_editorv3-cursors")[0];
...
testCM("lineChangeEvents", function(cm) {
  ...
  for (var i = 0; i < 5; ++i) {
    eronelit_editorv3.on(cm.getLineHandle(i), "delete", function(i) {
      return function() {log.push("del " + i);};
    }(i));
    eronelit_editorv3.on(cm.getLineHandle(i), "change", function(i) {
...
testCM("scrollEntirelyToRight", function(cm) {
  ...
  var wrap = cm.getWrapperElement(), cur = byClassName(wrap, "eronelit_editorv3-cursor")[0];
...
testCM("beforeChange", function(cm) {
  ...
  eronelit_editorv3.on(cm.getDoc(), "beforeChange", function(doc, change) {
...
testCM("lineStyleFromBlankLine", function(cm) {
  eronelit_editorv3.defineMode("lineStyleFromBlankLine_mode", function() {
...
eronelit_editorv3.registerHelper("xxx", "a", "A");
eronelit_editorv3.registerHelper("xxx", "b", "B");
eronelit_editorv3.defineMode("yyy", function() {
  return {
    token: function(stream) { stream.skipToEnd(); },
    xxx: ["a", "b", "q"]
  };
});
eronelit_editorv3.registerGlobalHelper("xxx", "c", function(m) { return m.enableC; }, "C");
...
testCM("combinedOperations", function(cm) {
  var place = document.getElementById("testground");
  var other = eronelit_editorv3(place, {value: "123"});
  try {
    cm.operation(function() {
      cm.addLineClass(0, "wrap", "foo");
...
test("core_rmClass", function() {
  ...
  eronelit_editorv3.rmClass(node, "quux");
  eq(node.className, "foo-bar baz-quux yadda");
  eronelit_editorv3.rmClass(node, "baz-quux");
  eq(node.className, "foo-bar yadda");
  eronelit_editorv3.rmClass(node, "yadda");
  eq(node.className, "foo-bar");
  eronelit_editorv3.rmClass(node, "foo-bar");
  eq(node.className, "");
  node.className = " foo ";
  eronelit_editorv3.rmClass(node, "foo");
  eq(node.className, "");
});

test("core_addClass", function() {
  var node = document.createElement("div");
  eronelit_editorv3.addClass(node, "a");
  eq(node.className, "a");
  eronelit_editorv3.addClass(node, "a");
  eq(node.className, "a");
  eronelit_editorv3.addClass(node, "b");
...
testCM("rtl_wrapped_selection", function(cm) {
  ...
  is(byClassName(cm.getWrapperElement(), "eronelit_editorv3-selected").length >= 3)
...
testCM("bidi_wrapped_selection", function(cm) {
  ...
  var blocks = byClassName(cm.getWrapperElement(), "eronelit_editorv3-selected")
  is(blocks.length >= 2)
  is(blocks.length <= 3)
...
}, {value: "  في تطبيق السمات مرة واحدة https://github.com/eronelit_editorv3/eronelit_editorv3/issues/4878#issuecomment-330550964على سبيل المثال <code>\"foo bar\"</code>\n" +
"  سيتم تعيين", direction: "rtl", lineWrapping: true});

eronelit_editorv3.defineMode("lookahead_mode", function() {
