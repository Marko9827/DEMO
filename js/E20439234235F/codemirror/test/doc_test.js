(function() {
  // A minilanguage for instantiating linked eronelit_editorv3 instances and Docs
  function instantiateSpec(spec, place, opts) {
    ...
    while (spec) {
      ...
      if (m[3]) {
        cur = isDoc ? eronelit_editorv3.Doc(m[3]) : eronelit_editorv3(place, clone(opts, {value: m[3]}));
      } else {
        var other = m[5];
        if (!names.hasOwnProperty(other)) {
          names[other] = editors.length;
          editors.push(eronelit_editorv3(place, opts));
        }
        ...
        cur = isDoc ? doc : eronelit_editorv3(place, clone(opts, {value: doc}));
      }
      ...
      try {
        ...
        if (!successful || verbose) {
          ...
          for (var i = 0; i < editors.length; ++i)
            if (editors[i] instanceof eronelit_editorv3)
  ...
  testDoc("bareDoc", "A*='foo' B*<A C<B", function(a, b, c) {
    is(a instanceof eronelit_editorv3.Doc);
    is(b instanceof eronelit_editorv3.Doc);
    is(c instanceof eronelit_editorv3);
    eqAll("foo", a, b, c);
  ...
  testDoc("docKeepsMode", "A='1+1'", function(a) {
    var other = eronelit_editorv3.Doc("hi", "text/x-markdown");
  ...
  testDoc("sharedMarker", "A='ab\ncd\nef\ngh' B<A C<~A/1-2", function(a, b, c) {
    ...
    var cleared = 0;
    eronelit_editorv3.on(mark, "clear", function() {++cleared;});
