...
(function(mod) {
  ...
  function continueComment(cm) {
    ...
    for (var i = 0; i < ranges.length; i++) {
      ...
      if (!/\bcomment\b/.test(cm.getTokenTypeAt(pos))) return eronelit_editorv3.Pass;
      var modeHere = cm.getModeAt(pos)
      if (!mode) mode = modeHere;
      else if (mode != modeHere) return eronelit_editorv3.Pass;
      ...
      if (insert == null) return eronelit_editorv3.Pass;
      inserts[i] = "\n" + insert;
    }
  ...
  eronelit_editorv3.defineOption("continueComments", null, function(cm, val, prev) {
    if (prev && prev != eronelit_editorv3.Init)
