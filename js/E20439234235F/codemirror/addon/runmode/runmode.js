...
eronelit_editorv3.runMode = function(string, modespec, callback, options) {
  var mode = eronelit_editorv3.getMode(eronelit_editorv3.defaults, modespec);
  var ie = /MSIE \d/.test(navigator.userAgent);
  var ie_lt9 = ie && (document.documentMode == null || document.documentMode < 9);

  if (callback.appendChild) {
    var tabSize = (options && options.tabSize) || eronelit_editorv3.defaults.tabSize;
  ...
  var lines = eronelit_editorv3.splitLines(string), state = (options && options.state) || eronelit_editorv3.startState(mode);
